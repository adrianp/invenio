...
def extract_notes_from_comment(comment, bodyOnly=False):
    ...
    for note in notes:
        ...
        marker = note[0]
        sub_note = extract_notes_from_comment(note[1], True)
        if len(sub_note) > 0:
            body = sub_note[0]['what']
            marker += '-' + sub_note[0]['where']['marker']
        else:
            body = note[1]
        result = {'what': body,
                  'where': {'marker': marker}}
...
def prepare_notes(notes):
    tree = {}
    for note in notes:
        markers = note["where"]["marker"].split("-")
